			...
			>
				...
				>
					...
					{nodes.map((node) => {
						const isConnectSelected =
							tool === "connect" && selectedNode && selectedNode.id === node.id;
						const isOtherSelected =
							selectedNode && selectedNode.id === node.id && tool !== "connect";
              ...
              `}
								style={{
									...
									animation: isConnectSelected
										? "connectGlow 1.5s ease-in-out infinite"
										: "none",
								}}
							>
								{node.label || node.id}
					...
					{nodes.length === 0 && (
						<div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
							<p className="text-zinc-500 text-lg color-white text-center">
								Selecciona "Agregar Nodo" y haz clic para comenzar
							</p>
						</div>
					)}
				</div>
